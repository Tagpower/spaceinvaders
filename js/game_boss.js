...
var player, enemies, bosses, items, shots, special_shots, enemy_shots, bonusships, explosions, shield; //Groups
...
function create() {
	...
	text_boss = game.add.text(180, game.world.height/2, 'DANGER : BOSS', {font: '32px Minecraftia', fill: '#ff2222'});
	text_boss.fixedToCamera = true;
	text_boss.alpha = 0;
...
function update() {
	...
	game.physics.arcade.collide(shots, bosses, hitBoss, null, this);
	game.physics.arcade.collide(special_shots, bosses, hitBoss, false, this); 
...
function loadBossLevel(lvl){
	in_boss_level = true;
	//alarm
	game.add.tween(text_boss).to( { alpha: 1 }, 400, Phaser.Easing.Linear.None, true, 0, 1000, true);
